...
class HumidityWidget : GlanceAppWidget() {

    override val stateDefinition: GlanceStateDefinition<HumidityWidgetModel>
        get() = object : GlanceStateDefinition<HumidityWidgetModel> {
            override suspend fun getDataStore(
                context: Context,
                fileKey: String,
            ): DataStore<HumidityWidgetModel> {
                val glanceWidgetUpdaters =
                    getFromEntryPoint<IGlanceWidgetUpdaterEntryPoint, Set<IGlanceWidgetUpdater>>(
                        context = context,
                    ) { getGlanceWidgetUpdater() }

                val humidityUpdater = glanceWidgetUpdaters
                    .filterIsInstance<HumidityGlanceWidgetUpdaterRepo>()
                    .firstOrNull()
                    ?: throw IllegalArgumentException("HumidityGlanceWidgetUpdaterRepo not found")

                return humidityUpdater
            }

            override fun getLocation(context: Context, fileKey: String): File {
                throw NotImplementedError("Not implemented for HumidityWidget State Definition")
            }
        }
